...
function setupMessageBox() {
	...
	message.innerHTML = "<b>Captcha çözülüyor...</b>";
...
function makeRequest(result){
	setupTextarea();
	sendMessage("<b>Captcha çözülüyor...</b>");
	let key = result.apiKey;
	let url = `https://2captcha.com/in.php?key=${key}&googlekey=${id}&method=userrecaptcha&pageurl=${pageurl}`;
	let http = new XMLHttpRequest();
	http.onreadystatechange = function() {
		if(http.readyState == 4 && http.status == 200){
			let response = http.responseText;
			if(response == messageError){
				sendMessage("Sağlanan API anahtarı hatalı.")
			}else{
				startWatching(response.substring(3), key);
			}
		}else if(http.readyState == 4){
			sendMessage("2captcha.com tarafında hata meydana geldi!");
		}
	}
	http.open("GET", url, true);
	http.send(null);
